...
The CLI SHALL provide a `network add` subcommand that accepts a user-supplied name and a node endpoint, connects to the node, derives the genesis block hash, and persists a named network entry to the durable config store.

#### Scenario: Successfully register a new network
- **WHEN** the user runs `ccd-wallet network add --name <NAME> --node <ENDPOINT>` and the name does not already exist in the config
