...
- **WHEN** the user provides `--node <ENDPOINT>` to a node command or to `transaction show`
- **THEN** the CLI connects directly to that endpoint

#### Scenario: Named network resolves to its stored endpoint
- **WHEN** the user provides `--network <NAME>` to a node command or to `transaction show`
...
- **WHEN** the user runs a node command or `transaction show` without `--network` or `--node`
...
- **WHEN** the user provides both `--network` and `--node` to a node command or to `transaction show`
- **THEN** the CLI exits with a non-zero status
