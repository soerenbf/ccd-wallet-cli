//! Transport abstraction for APDU exchange with a Governance Ledger device.
...
/// Minimal transport interface required by Governance Ledger command logic.
///
...
/// # Errors
///
/// Implementations return transport errors when commands cannot be exchanged with the device.
///
/// # Examples
///
/// ```
/// use ccd_wallet_ledger_governance::{ApduCommand, GovernanceLedgerError, GovernanceLedgerTransport};
///
/// struct Echo;
/// impl GovernanceLedgerTransport for Echo {
///     fn exchange_raw(&mut self, command: &ApduCommand) -> Result<Vec<u8>, GovernanceLedgerError> {
...
/// }
/// ```
pub trait GovernanceLedgerTransport {
    ...
    /// # Errors
    ///
    /// Returns an error if transport exchange fails, the reply is malformed, or the device
    /// returns a non-success status word.
...
impl MockTransport {
    ...
    ///
    /// ```
    /// use ccd_wallet_ledger_governance::MockTransport;
...
impl GovernanceLedgerTransport for MockTransport {
    fn exchange_raw(&mut self, command: &ApduCommand) -> Result<Vec<u8>> {
        self.commands.push(command.clone());
        self.replies
            .pop_front()
            .ok_or_else(|| GovernanceLedgerError::transport("mock transport has no queued reply"))
    }
}
...
mod hid {
    ...
    impl HidTransport {
        ...
        ///
        /// ```no_run
        /// use ccd_wallet_ledger_governance::HidTransport;
        /// let _transport = HidTransport::open_first()?;
        /// # ccd_wallet_ledger_governance::Result::Ok(())
        /// ```
        pub fn open_first() -> Result<Self> {
            let api =
                HidApi::new().map_err(|err| GovernanceLedgerError::transport(err.to_string()))?;
            let device_info = api
                .device_list()
                .find(|device| {
                ...
                .ok_or_else(|| GovernanceLedgerError::transport("no Ledger HID device found"))?;
            let device = device_info
                .open_device(&api)
                .map_err(|err| GovernanceLedgerError::transport(err.to_string()))?;
            Ok(Self { device })
        }
    ...
    impl GovernanceLedgerTransport for HidTransport {
        fn exchange_raw(&mut self, command: &ApduCommand) -> Result<Vec<u8>> {
    ...
    fn write_framed(device: &HidDevice, apdu: &[u8]) -> Result<()> {
        let apdu_len = u16::try_from(apdu.len()).map_err(|_| {
            GovernanceLedgerError::invalid_request(format!(
                "encoded APDU is too large: {} bytes",
                apdu.len()
            ))
        })?;
        let mut offset = 0;
        ...
        while offset < apdu.len() || sequence == 0 {
            ...
            sequence = sequence
                .checked_add(1)
                .ok_or_else(|| GovernanceLedgerError::transport("HID sequence overflow"))?;
        }
        Ok(())
    }
    ...
    fn read_framed(device: &HidDevice) -> Result<Vec<u8>> {
        ...
        loop {
            ...
            if (sequence == 0 && size < 7) || size < 5 {
                return Err(GovernanceLedgerError::transport("short Ledger HID packet"));
            }
            if u16::from_be_bytes([packet[0], packet[1]]) != CHANNEL || packet[2] != TAG_APDU {
                return Err(GovernanceLedgerError::transport(
            ...
            let expected_len = expected_len.ok_or_else(|| {
                GovernanceLedgerError::transport("missing Ledger HID response length")
            })?;
