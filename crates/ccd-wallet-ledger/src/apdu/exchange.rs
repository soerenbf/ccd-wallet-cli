...
/// A complete APDU command for the Concordium Ledger app.
...
impl ApduCommand {
    /// Construct a Concordium Ledger app command using the default CLA byte.
    ///
    /// # Arguments
    ///
    ...
    ///
    /// ```
    /// use ccd_wallet_ledger::apdu::ApduCommand;
    /// let command = ApduCommand::new(0x01, 0x00, 0x00, vec![1, 2]);
    /// assert_eq!(command.cla, ccd_wallet_ledger::apdu::LEDGER_CLA);
    /// ```
...
impl ApduReply {
    ...
    ///
    /// ```
    /// use ccd_wallet_ledger::apdu::ApduReply;
